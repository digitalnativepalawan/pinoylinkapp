    ...
    <div className={`min-h-screen ${bgClass}`}>
      <div className="mx-auto max-w-md px-5 pb-16 pt-14">
        <div className="flex flex-col items-center text-center">
          <div
            className={`h-28 w-28 overflow-hidden rounded-full border-4 ${isDark ? "border-white/80" : "border-white"} shadow-xl`}
          >
            {p.avatar_url ? (
          ...
          <div
            className={`mt-3 inline-block rounded-full px-3 py-0.5 text-[11px] font-semibold text-white ${p.status === "online" ? "bg-emerald-500" : "bg-zinc-500"}`}
          >
          ...
          {p.bio && (
            <p
              className={`mt-1 whitespace-pre-line text-sm ${isDark ? "text-white/80" : "text-zinc-600"}`}
            >
              {p.bio}
            </p>
          )}
          ...
          {data.links.map(
            (l: {
              ...
              return (
                ...
                >
                  <div
                    className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${isDark ? "bg-white/10" : "bg-zinc-50"}`}
                  >
                    <Icon
                      className={`h-5 w-5 ${l.color || (isDark ? "text-white" : "text-zinc-700")}`}
                    />
                  </div>
                  <div className="min-w-0 flex-1 truncate text-sm font-semibold">{l.label}</div>
                  <ChevronRight
                    className={`h-4 w-4 ${isDark ? "text-white/60" : "text-zinc-400"}`}
                  />
                </button>
              );
            },
