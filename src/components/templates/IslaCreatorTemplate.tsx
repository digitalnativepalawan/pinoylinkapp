...
const ROW_COLORS = [
  { bg: NAVY, text: "text-white", sub: "text-[#e8b93a]", iconBg: "bg-white", iconText: `text-[${NAVY}]`, chev: "text-white" },
  { bg: RED, text: "text-white", sub: "text-[#e8b93a]", iconBg: "bg-white", iconText: `text-[${RED}]`, chev: "text-white" },
  { bg: YELLOW, text: "text-[#0a2e6b]", sub: "text-[#0a2e6b]/70", iconBg: "bg-white", iconText: `text-[${NAVY}]`, chev: "text-[#0a2e6b]" },
  { bg: NAVY, text: "text-white", sub: "text-[#e8b93a]", iconBg: "bg-white", iconText: `text-[${NAVY}]`, chev: "text-white" },
  { bg: "#ffffff", text: "text-[#c92030]", sub: "text-[#0a2e6b]/70", iconBg: `bg-[${RED}]`, iconText: "text-white", chev: "text-[#0a2e6b]" },
];
...
function BrushCornerNavy({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 300 200" className={className} preserveAspectRatio="none">
      <path
        d="M0,0 L300,0 Q260,40 210,50 Q140,65 80,90 Q30,110 0,150 Z"
        fill={NAVY}
      />
      <g fill={YELLOW}>
        ...
        {Array.from({ length: 10 }).map((_, i) => {
          ...
          return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={YELLOW} strokeWidth={4} strokeLinecap="round" />;
        })}
      </g>
...
function BrushCornerRed({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 300 200" className={className} preserveAspectRatio="none">
      <path
        d="M300,0 L300,180 Q260,150 220,120 Q170,80 130,40 Q100,10 60,0 Z"
        fill={RED}
      />
    </svg>
  );
    ...
    <svg viewBox="0 0 100 100" width={size} height={size}>
      <g>
        {Array.from({ length: 5 }).map((_, i) => {
          ...
          return <ellipse key={i} cx={cx} cy={cy} rx={14} ry={20} fill="white" stroke="#e8dfd0" strokeWidth={0.8} transform={`rotate(${(i * 72) - 90} ${cx} ${cy})`} />;
        })}
      ...
      <div className="relative mx-auto max-w-md px-6 pb-20 pt-20">
        ...
        <div className="mx-auto flex flex-col items-center">
          <div className="relative">
            ...
            <div className="relative m-3 h-32 w-32 overflow-hidden rounded-full border-[3px]" style={{ borderColor: YELLOW, background: CREAM }}>
              {profile.avatar_url ? (
                <img src={profile.avatar_url} alt={profile.display_name} className="h-full w-full object-cover" />
              ) : (
                <div className="grid h-full w-full place-items-center text-4xl" style={{ color: NAVY }}>✿</div>
              )}
            </div>
            {/* Star sparkles */}
            <div className="absolute -left-2 top-6 text-lg" style={{ color: YELLOW }}>✦</div>
            <div className="absolute -right-1 bottom-8 text-base" style={{ color: YELLOW }}>✦</div>
          </div>
        </div>
        ...
        <div className="mt-6 text-center">
          <h1
            ...
            style={{ fontFamily: '"Brush Script MT", "Snell Roundhand", "Dancing Script", cursive', color: NAVY }}
          >
            {profile.display_name}
            <span className="ml-2 text-xl" style={{ color: YELLOW }}>☀</span>
          </h1>
          <div className="mt-3 text-[11px] font-bold uppercase tracking-[0.35em]" style={{ color: RED }}>
        ...
        {/* Bio */}
        {profile.bio && (
          <p className="mx-auto mt-4 max-w-xs text-center text-sm leading-relaxed" style={{ color: NAVY }}>
            {profile.bio}
            <br />
            <span className="italic" style={{ color: NAVY, fontFamily: '"Brush Script MT", cursive' }}>
        ...
        <div className="mt-8 space-y-3">
          {links.map((l, i) => {
            ...
            return (
              ...
              >
                <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-full ${scheme.iconBg}`}>
                  <Icon className={`h-5 w-5 ${scheme.iconText}`} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className={`truncate text-[15px] font-extrabold uppercase tracking-wide ${scheme.text}`}>
                    {l.label}
                  </div>
                  <div className={`truncate text-xs ${scheme.sub}`}>
                    {host || "Tap to open"}
                  </div>
                </div>
        ...
        <div className="mt-10 text-center">
          <div className="text-xl" style={{ color: YELLOW }}>♥</div>
          <div
          ...
          <div className="mt-1 text-[10px] font-semibold uppercase tracking-[0.3em]" style={{ color: NAVY }}>
